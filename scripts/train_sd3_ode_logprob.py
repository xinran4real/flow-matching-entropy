...
def compute_log_prob(transformer, pipeline, sample, j, embeds, pooled_embeds, config):
    ...
    return prev_sample, log_prob, prev_sample_mean, std_dev_t, noise_pred

def compute_log_prob_ref(transformer, pipeline, sample, j, embeds, pooled_embeds, config):
    if config.train.cfg:
        noise_pred = transformer(
            hidden_states=torch.cat([sample["latents"][:, j]] * 2),
            timestep=torch.cat([sample["timesteps"][:, j]] * 2),
...
def compute_ode_log_prob(transformer, pipeline, sample, j, embeds, pooled_embeds, config):
    """使用有限差分方法计算 ODE log prob"""
    # 1. Calculate v(x)
    if config.train.cfg:
        noise_pred = transformer(
            hidden_states=torch.cat([sample["latents"][:, j]] * 2),
    ...
    if hasattr(config, "calculate_log_prob") and config.calculate_log_prob:
        ...
        # C. Perturbed input
        latents_perturbed = sample["latents"][:, j] + h * epsilon
        
        # D. Calculate v(x + h*epsilon)
        if config.train.cfg:
            noise_pred_perturbed = transformer(
                hidden_states=torch.cat([latents_perturbed] * 2),
                timestep=torch.cat([sample["timesteps"][:, j]] * 2),
                encoder_hidden_states=embeds,
                pooled_projections=pooled_embeds,
                return_dict=False,
        ...
        log_prob_velocity = None
    # -----------------------------------------------------------

    # compute the log prob of next_latents given latents under the current model
    if hasattr(config, "calculate_log_prob") and config.calculate_log_prob:
        prev_sample, log_prob, prev_sample_mean, std_dev_t, ode_log_prob = sde_step_with_logprob(
            pipeline.scheduler,
            noise_pred.float(),
            sample["timesteps"][:, j],
            sample["latents"][:, j].float(),
        ...
        return prev_sample, log_prob, prev_sample_mean, std_dev_t, ode_log_prob, noise_pred, log_prob_velocity
    else:
        prev_sample, log_prob, prev_sample_mean, std_dev_t = sde_step_with_logprob(
        pipeline.scheduler,
        noise_pred.float(),
    ...
    while True:
        ...
        for i in tqdm(
            ...
            with autocast():
                with torch.no_grad():
                    if hasattr(config, "calculate_log_prob") and config.calculate_log_prob:
                        images, latents, log_probs, epsilons, ode_log_probs = pipeline_with_logprob(
                            pipeline,
                            prompt_embeds=prompt_embeds,
                            pooled_prompt_embeds=pooled_prompt_embeds,
                            negative_prompt_embeds=sample_neg_prompt_embeds,
                            ...
                            noise_level=config.sample.noise_level,
                            generator=generator,
                            calculate_log_prob=config.calculate_log_prob
                        )
                    else:
            ...
            if hasattr(config, "calculate_log_prob") and config.calculate_log_prob:
                epsilons = torch.stack(epsilons, dim=1) # (batch_size, num_steps, 16, 32, 32)
                ode_log_probs = torch.stack(ode_log_probs, dim=1) # shape after stack (batch_size, num_steps)
            ...
            samples.append(
                {
                    ...
                    "log_probs": log_probs,
                    "ode_log_probs": ode_log_probs if hasattr(config, "calculate_log_prob") and config.calculate_log_prob else None,
                    "epsilons": epsilons if hasattr(config, "calculate_log_prob") and config.calculate_log_prob else None,
                    "rewards": rewards,
                }
            )
            ...
            for i, sample in tqdm(
                ...
                for j in tqdm(
                    ...
                    with accelerator.accumulate(transformer):
                            ...
                            if config.train.beta > 0:
                                with torch.no_grad():
                                    with transformer.module.disable_adapter():
                                        _, _, prev_sample_mean_ref, std_dev_t, noise_pred_ref = compute_log_prob_ref(transformer, pipeline, sample, j, embeds, pooled_embeds, config)
                            if hasattr(config, "calculate_log_prob") and config.calculate_log_prob:
                                prev_sample, log_prob, prev_sample_mean, _, ode_log_prob, noise_pred, log_prob_velocity = compute_ode_log_prob(transformer, pipeline, sample, j, embeds, pooled_embeds, config)
                            else:
                                prev_sample, log_prob, prev_sample_mean, std_dev_t, noise_pred = compute_log_prob(transformer, pipeline, sample, j, embeds, pooled_embeds, config)
                        ...
                        if hasattr(config, "calculate_log_prob") and config.calculate_log_prob:
                            ratio = torch.exp(ode_log_prob - sample["ode_log_probs"][:, j])
                        else:
                        ...
                        if config.train.beta > 0:
                            if hasattr(config, "calculate_log_prob") and config.calculate_log_prob:
                                kl_loss = ((noise_pred - noise_pred_ref) ** 2).mean()
                            else:
                        ...
                        info["div_magnitude"].append(
                            torch.mean(log_prob_velocity.abs())
                        )

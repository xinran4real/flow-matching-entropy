...
def sde_step_with_logprob(
    ...
    log_prob_velocity: Optional[torch.FloatTensor] = None,
):
    """
    ...
    if log_prob_velocity is not None:
        # log_p_{t-1} = log_p_t + dt * (-div_t)
        ode_log_prob = log_prob_velocity * dt
        if return_sqrt_dt:
            return prev_sample, log_prob, prev_sample_mean, std_dev_t, torch.sqrt(-1*dt), ode_log_prob
        return prev_sample, log_prob, prev_sample_mean, std_dev_t, ode_log_prob
    else:
        if return_sqrt_dt:

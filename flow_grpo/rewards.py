...
def pickscore_score(device):
    ...
    def _fn(images, prompts, metadata, use_reward_gradient=False):
        if use_reward_gradient:
            scores = scorer(prompts, images, use_reward_gradient)
            if not metadata[0]["requires_grad"]:
...
def multi_score(device, score_dict):
    ...
    def _fn(images, prompts, metadata, ref_images=None, only_strict=True, use_reward_gradient=False):
        total_scores = []
        score_details = {}
        ...
        for score_name, weight in score_dict.items():
            if score_name == "geneval":
                ...
                scores, rewards = score_fns[score_name](images, prompts, metadata, use_reward_gradient=use_reward_gradient)
            else:

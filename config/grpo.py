...
def pickscore_sd3_1batch_size():
    ...
    config.sample.num_image_per_prompt = 4 # sample a group
    ...
    config.sample.same_latent = False
    config.train.ema = True
    config.save_freq = 10000000 # epoch
    config.eval_freq = 10000000
    config.save_dir = 'logs/pickscore/sd3.5-M'
    config.reward_fn = {
        "pickscore": 1.0,
    }
    ...
    config.per_prompt_stat_tracking = True
    config.train.learning_rate = 1e-4
    config.use_reward_gradient = False
    return config

def pickscore_sd3_1batch_size_use_gradient():
    gpu_number=4
    config = compressibility()
    config.dataset = os.path.join(os.getcwd(), "dataset/pickscore")
    config.eval_dataset = os.path.join(os.getcwd(), "dataset/ShareGPT4V")
    ...
    config.resolution = 256
    config.sample.train_batch_size = 1
    config.sample.num_image_per_prompt = 1 # no need to sample a group
    config.sample.num_batches_per_epoch = 1 # keep the original value  int(16/(gpu_number*config.sample.train_batch_size/config.sample.num_image_per_prompt))
    # assert config.sample.num_batches_per_epoch % 2 == 0, "Please set config.sample.num_batches_per_epoch to an even number! This ensures that config.train.gradient_accumulation_steps = config.sample.num_batches_per_epoch / 2, so that gradients are updated twice per epoch."
    config.sample.test_batch_size = 4 # This bs is a special design, the test set has a total of 2048, to make gpu_num*bs*n as close as possible to 2048, because when the number of samples cannot be divided evenly by the number of cards, multi-card will fill the last batch to ensure each card has the same number of samples, affecting gradient synchronization.
    ...
    config.per_prompt_stat_tracking = True
    config.train.learning_rate = 1e-4
    config.use_reward_gradient = True
    config.eval_diversity = False
    config.eval_diversity_freq = 50
    config.num_seeds = 1
    config.eval_ode_log_prob = True
    config.eval_ode_log_prob_freq = 50
    config.visualize_steps = True
    config.visualize_steps_freq = 50
    return config

def pickscore_sd3_fast_4gpu():
    gpu_number=4
    config = compressibility()
    config.dataset = os.path.join(os.getcwd(), "dataset/pickscore")
    ...
    config.sample.noise_level = 0.8
    config.train.ema = True
    config.save_freq = 1000000 # epoch
    config.eval_freq = 60
...
def pickscore_sd3_4gpu_ode_logprob():
    gpu_number=4
    config = compressibility()
    config.dataset = os.path.join(os.getcwd(), "dataset/pickscore")
    ...
    config.train.learning_rate = 1e-4
    config.calculate_log_prob = True
    return config
